...
security validation, Hub VAR packaging, Windows release build, branding, Setup
creation, and CI artifact upload. `.github/workflows/codeql.yml` performs Rust
security analysis. `.github/workflows/release.yml` validates a versioned tag
and repeats the release gates. The CLR 2 plugin is a hash-locked baseline that
...
Public repository history uses the collective maintainer pseudonym `VaMender`
for project-authored commits and annotated tags. A repository-local setup tool
...
GitHub's authenticated event actor, so maintainers must push through a separate
pseudonymous GitHub account or GitHub App rather than a personal credential.
See REQ-027.
